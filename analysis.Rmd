...
packages <- c("dplyr","readxl", "curl", "ggplot2", "ggrepel", "maps", "plotly", "stringr", "tm", "wordcloud2", "tidyverse", "RColorBrewer", "ggwordcloud", "viridis", "bookdown", "utils", "leaps", "broom")
...
For the following work, different R Packages have been used. The following code shows, which packages have been included and an efficient way how to include multiple packages and for the work in a group. 

# !!! should we include your fancy package set up code here?

# !!! should we include here a graphical overview of our data? 
...
While looking closer into the restaurant data we found a restaurant without a Zip code. We looked up the respectiv restautant and found the right zip code for the restaurant. With the following code we cleaned the missing data from on restaurant and added the zip code to the data set. 
...
```{r}
#summarize vegan & vegetarian searches
...
```{r}
#Vegan and Vegetarian Restaurants by State
...
#temp <- joined_data %>% distinct(id, province, Mean) %>% group_by(province) %>% summarise(Avg_Mean_Province = (sum(Mean) / n_distinct(Mean))) 
#ggplot(data=temp, aes(x=province, y=Avg_Mean_Province, fill=province)) +
#  geom_boxplot(stat="identity") +
#  ggtitle("Yearly Mean Household Income by State") + xlab("State") + ylab("Average Mean Household Income") + labs(fill='State') +
#  theme_minimal()


#head(income_pop_by_zip)
#p <- ggplot2(income_pop_by_zip, aes(x= Mean, y = Zip, fill = feed))
#p <- p + geom_boxplot()
#p <- p + theme_classic()
...
```{r}
# Median Household income by state !! Help
# income_box <- read_excel("./Data/MeanZIP-3.xlsx")
# ggplot(income_box,
  #     aes(x = estimate,
...
```{r, echo=FALSE}
pairs(unique_res_data %>% select(`Number of Restaurants in State`, `Population Size at Zip Code`, `Mean Household Income at Zip Code`, `Google Search Frequency for Vegetarian Restaurant`, `Google Search Frequency for Vegan Restaurant` ))
``` 
...
\newpage
## Maps and Wordcloud | Chapter of Choice
...
\newpage
# Chater of Choice: Maps, Wordcloud and Bookdown
