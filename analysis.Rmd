...
For the following work different R Packages have been used. The following code shows, which packages have been included and a efficient way how to include multiple packages and for the work in a group. 
...
The following work is based on three different data sources. We are gathering data from Datafinit through Kaggel (Restaurant Data), Kaggel (Mean household income U.S.) and from Google Trend (Google Search Data).
...
income_pop_by_zip %>% group_by(Zip)  %>% summarize(mean=mean(Mean,na.rm=TRUE), std_dev =sd(Mean,na.rm=TRUE))
```
...
ggplot(google_search_frequency, aes(x=province, y=google_search_frequency, fill=province)) + 
  geom_bar(stat = "identity") +
  ggtitle("Google Searches by State") + xlab("State") + ylab("Number of Google Searches") + labs(fill='State') +
  theme_minimal()
```
...
number_of_res <- joined_data %>% group_by(province) %>% summarise(Number_Of_Restaurants = n_distinct(id))
ggplot(number_of_res, aes(x=province, y=Number_Of_Restaurants, fill=province)) + 
  geom_bar(stat = "identity") +
  ggtitle("Number of Restaurants by State") + xlab("State") + ylab("Number of Restaurants") + labs(fill='State') +
  theme_minimal()
```
...
```{r}
#Scatterplot
# ggplot(joined_data, aes(x = province, y = vegan_searches)) + geom_point()
