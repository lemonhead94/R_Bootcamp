...
```{r}
#mean household income whole U.S.
summary(income_pop_by_zip)
```


```{r}
#summarize household income in U.S. 
income_summary <- income_pop_by_zip %>%
  group_by(Zip)  %>%
...
#hist_restaurants_sub <- subset(restaurants, province)$name
#hist(hist_restaurants_sub, main = "Restaurants by Province",
 #    xlab = "Province")
```
...
```{r}
# Mean Household income by Zip !! Help
#ggplot2(income_pop_by_zip, aes(y = Mean, group = Zip, x = Zip)) + 
#  geom_boxplot(fill=c("green", "lightgreen", "darkgreen"))
```
